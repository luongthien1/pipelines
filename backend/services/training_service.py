...
from db.model.dataset import DatasetVersion
    ...
    try:
        ...
        if not db_model:
            ...
            if pipeline.dataset_version_id:
                from db.model.dataset import DatasetVersion as DV
                dv = db.query(DV).filter(DV.id == pipeline.dataset_version_id).first()
                if dv:
                    from db.model.dataset import Dataset as DS
        ...
        from db.model.model import ModelVersion as MV
...
def _run_detr_training(
    ...
    from db.model.dataset import DatasetItem, DatasetVersion as DV
