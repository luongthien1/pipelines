        ...
        {imgDims.w > 0 && (
          ...
          >
            ...
            {polygons.map((poly, pi) => {
              ...
              const pts = toSvgPoints(poly.points);
              return (
                <g key={pi} style={{ pointerEvents: 'auto' }}>
                  ...
                  {poly.points.length > 0 && (
                    <text
                      x={poly.points[0][0]}
                      y={poly.points[0][1] - 6 / zoom}
                      fill={color}
                      fontSize={12 / zoom}
                      fontWeight="bold"
                      style={{ userSelect: 'none', opacity: isSelected ? 1 : 0.7 }}
                    >
                      {poly.label}
                    </text>
                  )}
                  ...
                  {isSelected && poly.points.map((pt, vi) => (
                    <circle
                      ...
                      cx={pt[0]} cy={pt[1]}
                      r={5 / zoom}
                      fill={selectedVertexIdx === vi ? color : 'white'}
                      stroke={color}
                      strokeWidth={1.5 / zoom}
                      style={{ cursor: 'move', pointerEvents: 'auto' }}
                      onClick={(e) => { e.stopPropagation(); setSelectedVertexIdx(vi); }}
                    />
                  ))}
                </g>
              );
            ...
            {drawingPoints.length > 0 && (
              <g style={{ pointerEvents: 'none' }}>
                ...
                {drawingPoints.map((pt, i) => {
                  ...
                  return <line key={i} x1={prev[0]} y1={prev[1]} x2={pt[0]} y2={pt[1]} stroke={getLabelColor(activeLabel)} strokeWidth={1.5 / zoom} />;
                })}
                {/* Line to cursor */}
                ...
                {drawingPoints.map((pt, i) => {
                  ...
                  return (
                    <circle
                      key={i}
                      cx={pt[0]} cy={pt[1]}
                      r={isFirst ? 6 / zoom : 4 / zoom}
                      fill={canClose ? getLabelColor(activeLabel) : 'white'}
                      stroke={getLabelColor(activeLabel)}
                      strokeWidth={1.5 / zoom}
