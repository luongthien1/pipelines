 ...
 * Layout: Left Asset Strip, Right Layers Panel, Floating Action Toolbar (Right), Floating Dashboard (Left).
 */
...
// ── Toolbar Components (Replicated from Original) ──────────────────────────
...
const ToolbarBtn: React.FC<ToolbarBtnProps> = ({ onClick, disabled = false, colorClass, children }) => (
  <button
    onClick={onClick}
    disabled={disabled}
...
const TBIcon: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className="w-11 h-11 flex items-center justify-center shrink-0">{children}</span>
);

const TBLabel: React.FC<{ text: string; forceShow?: boolean }> = ({ text, forceShow = false }) => (
...
const LabelerShell: React.FC<ShellProps> = ({
  ...
  return (
    <div className="flex h-full w-full bg-[#0a0a0a] overflow-hidden relative group/studio shrink-0">
      {/* ── Left image strip ── */}
      <div className="w-12 hover:w-64 bg-black/80 backdrop-blur-3xl border-r border-white/5 flex flex-col shrink-0 transition-all duration-500 z-30 group/sidebar shadow-2xl">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-black relative p-6 h-full">
        <div className="relative h-full w-full bg-[#0c0c0c] rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl transition-colors duration-500">
          ...
          {/* Vertical action toolbar floating right */}
          <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-40">
            <ToolbarBtn 
              onClick={() => { if (currentIdx > 0) onSelectItem(items[currentIdx - 1].id); }} 
            ...
            <ToolbarBtn 
              onClick={() => { if (currentIdx < items.length - 1) onSelectItem(items[currentIdx + 1].id); else if (onLoadMore) onLoadMore(); }} 
              disabled={currentIdx === -1 || (currentIdx >= items.length - 1 && !onLoadMore)} 
              colorClass="text-white/50 hover:text-white"
            >
            ...
            <ToolbarBtn onClick={() => { if (layers.length > 0 && window.confirm('Clear all annotations?')) onClear(); }} disabled={layers.length === 0} colorClass="text-white/50 hover:text-red-400 hover:border-red-400/30">
            ...
            <ToolbarBtn onClick={onSave} disabled={isSaving} colorClass={saveSuccess ? 'text-blue-400 border-blue-500/30' : 'text-[var(--green)] border-[var(--green)]/30 hover:bg-[var(--green)]/10'}>
              ...
              <TBIcon>
                {isSaving
                  ? <div className="w-4 h-4 border-2 border-[var(--green)]/30 border-t-[var(--green)] rounded-full animate-spin" />
                  : saveSuccess ? <CheckCircle2 className="w-5 h-5 text-blue-400" /> : <Save className="w-5 h-5" />
                }
              </TBIcon>
            </ToolbarBtn>

            <ToolbarBtn onClick={onCancel} colorClass="text-white/50 hover:text-red-400 hover:border-red-400/30">
      ...
      <div className={`${rightExpanded ? 'w-72' : 'w-14'} h-full bg-[#080808] border-l border-white/5 flex flex-col shrink-0 z-30 transition-all duration-500`}>
        <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0">
          <div className={`transition-opacity duration-300 ${rightExpanded ? 'opacity-100' : 'opacity-0 w-0 overflow-hidden'}`}>
            ...
            <span className="text-[8px] font-black text-white/20 tracking-widest uppercase">{layers.length} regions</span>
          </div>
          <button 
        ...
        <div className={`flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar transition-opacity duration-300 ${rightExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} style={{ maxHeight: 'calc(100% - 80px)' }}>
          ...
          {layers.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center opacity-20 py-20 text-center">
              <div className="w-8 h-8 mb-4 border-2 border-dashed border-white/20 rounded-full flex items-center justify-center">
                <RotateCcw className="w-4 h-4" />
              </div>
              <p className="text-[10px] font-bold uppercase tracking-widest leading-loose">Waiting for<br />annotations</p>
            </div>
          )}
        </div>
      </div>
